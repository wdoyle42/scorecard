...
## State fips 
state_fips<-read.csv(paste0(ddir,'state_fips.csv'))

names(state_fips)<-c("stfips","longname","stabbr")

state_fips$stfips<-sprintf("%02d",state_fips$stfips)
...
krigeMap <- function(ddir,                          # data directory
    ...
    ## subset to 50 states
    county_shape <- county_shape[county_shape@data$state %in% state_fips$stfips,]
    ...
    ## Miscellaneous cleanup and transformations
    ...
    mapname <- paste0(mdir, 'county_shape_', outcome_var,'_',groupname, '_', year, '.geojson' )
    if(save.file) {
  ...
  ## attendee palette
   ...
   pct_pal  <- colorQuantile(pal = my.pal,
    ...
    inst_popup <- paste0(as.character(instlayer@data$instnm.x),
                         ## so kludgey it makes my eyeballs bleed
                         var_describe,
...
##separate kriged maps for different institution types
...
## If file already exists, don't run: if you want to rerun, just delete or move files

if (file.exists(paste0(mdir,"county_shape_md_earn_wne_p10_pub2_2011.geojson"))==FALSE){
#Public 2 year
  ...
  }else print("File exists, no new estimation")
  

#Public 4 year
if (file.exists(paste0(mdir,"county_shape_md_earn_wne_p10_pub4_2011.geojson"))==FALSE){
  krigeMap(dat=inst_dat,
           inst_sp=inst_sp,
           acsdat=acs,
           select_group=c(1,2),
...
priv4_map<-mapSC(countylayer=county_shape,
                instlayer=inst_sp[inst_sp@data$group==c(3,4),],
                var_describe=var_describe,
                outcome_var=outcome)
...
-The "payoff" for college has a strong regional component. Young people living in the south do not generally live close to
any community colleges that have high attendee incomes. 
-Regional variation in earnings is closely tied to proximity to strong labor markets: institutions in Chicago, New York and the Bay Area all are in the top deciles of earnings. 
-The role of community colleges in economic mobility also stands out. In many counties, community college attendees earn substantially more than the median income. These tend (not surprisingly) to be poorer counties, likely with low overall levels of educational attainment. 
...
## County Level Estimates: Earnings of Private Four-Year Students Ten Years After Entry
```{r printmap_priv4, results = 'show', fig.width = 9.5}
################################################################################
## PRINT MAP
################################################################################
