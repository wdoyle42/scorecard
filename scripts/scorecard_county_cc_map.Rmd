...
```{r,echo=F,warning=F,message=F,comment=F,include=F,results='hide'}
################################################################################
...
## get ACS
acs <- read_csv(paste0(ddir, 'acs_medinc_2012_3yr.csv'))
...
krigeMap <- function(ddir,                          # data directory
    ...
    county_shape <- county_shape[county_shape$statefp %in% dat$stfips,] 
...
mapSC <- function(countylayer,          # county map data
    ...
    inst_popup <- paste0(as.character(instlayer@data$instnm),
                         ...
                         prettyNum(instlayer[[outcome_var]],
                                   ...
                                   scientific = FALSE,
                                   digits = 0),
                         '<br/> Percent of median income: ',
                         instlayer@data$pctmedinc,
                         '%'
                         )
...
countyMapCS <- function(csdat,               # scorecard data
    ...
    mapdat <- instdat %>%
        ...
        mutate(pctinc = round(outcome_var / medinc * 100, 2)) 
