...
instPointsSpatial <- function(dat,                  # merged score data
    ...
    ## filename <- paste0(mdir, 'institutional_points.Rda')
...
krigeMap <- function(ddir,                          # data directory
    ...
    ## simplify shapefile to reduce size
    county_shape_simple <- gSimplify(county_shape,
                                     tol = .001,
                                     topologyPreserve = TRUE)

    ## create a spatial polygons dataframe
    county_shape <- SpatialPolygonsDataFrame(county_shape_simple,
...
mapSC <- function(countylayer,          # county map data
    ...
    if(split > 3) {
         ...
         legend_title <- paste(paste(legend_title[1:split], collapse = ' '),
                               '<br/>',
                               paste(legend_title[split:length(legend_title)],
                                     collapse = ' '), collapse = '')
    } else {
    ...
    county_popup <- paste0(countylayer@data$namelsad,
                           var_describe, 
    ...
    county_map <- leaflet(countylayer) %>%
        ...
        addCircleMarkers(lng = instlayer@coords[,1],
...
countyMapCS <- function(csdat,               # scorecard data
    ...
    ## drop states list
    dslist <-  c('AK','AS','DC','FM','GU','HI','MP','PR')
